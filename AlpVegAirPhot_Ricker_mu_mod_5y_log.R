...
veg[,2] <- floor((veg$date-1931)/5)
################this recodes dates into 5yr intervals###################

names(veg[,6]) <- "other.hits"
sites <- read.csv('Alpsites.csv')   # read in from data folder
#yrs <- veg$date-1970
hd <- floor((veg$date-1931)/5)
veg <- merge(veg,sites)
veg <- veg[with(veg,order(veg$Site, veg$date)), ]
veg[,7] <- floor((veg[,7]-1931)/5)
first <- min(veg[,2])
last <- max(veg[,2])
...
tsg <- floor((sites$last.grazed-1931)/5)
t <- all.yrs.a[,,1]
for (i in 1:23){
  for (j in 1 : 14 ) {
...
veg.inits <- function() list(
 # mn.K = c(3,3,3,3),
  alpha.100 = c(0.5,1,0.5,1), #rnorm(3, mean = 0.05, sd = 0.01),
  N = N.init,
 # KK = KK.init, #runif(23, 500, 1500),
  sd = runif(1,0.5,1), 
  gr.ef = rnorm(4, mean = 0.5, sd = 0.1),
  cb.ef = rnorm(4, mean = -0.05, sd = 0.1),
  f.ef = rnorm(4, mean = -0.5, sd = 0.1),
 a = c(3,2.5,3.5, 3),
 b = c(0,0,0, 0)
...
veg.data <- list( 
  ...
  N = all.yrs.a,
  sg = as.matrix(t[ , 1:14]),
  cb.t = as.matrix(cb.t[ , 1:14]),
  fy.t = as.matrix(fy.t[ , 1:14])) ## , 
...
for (j in 1:Nsites) {
    ...
    for (t in yoo1[j] : T) { 
      ...
      for (k in 1:Nveg) {
       ...
       # write a prediction
      #   Np[j, t+1] <- N[j, t] * (Er[j, t] + gr.ef * sg[j, t] )
    } 
     for (k in 1:Nveg-1) {
        log.K[j, t, k] ~ dnorm(mu.K[j,t,k], sd^-2)T(0,7.4)
}
    mu.K[j,t,1]  <-  KK[j, 1]  + f.ef[1] * fy.t[j, t] + gr.ef[1] * sg[j, t]  +  cb.ef[1] * cb.t[j, t] 
       mu.K[j,t,2]  <-  KK[j, 2]  + f.ef[2] * fy.t[j, t] + gr.ef[2] * sg[j, t]  +  cb.ef[2] * cb.t[j, t]
       mu.K[j,t,3]  <-  KK[j, 3]  + f.ef[3] * fy.t[j, t] + gr.ef[3] * sg[j, t]   + cb.ef[3] * cb.t[j, t]
...
for (k in 1:4) {
  ...
  for (j in 1: Nsites) { 
    for (k in 1: 3) {
      KK[j,k] <- a[k] + b[k] * (precip[j] - 1465)/216 # (alt[j]-1433)/185  #standardize
 # ~ dnorm(mn.K[k], sd[k]^-2)T(0,7.38) #~ dnorm(4,1) #~ dunif(0, 7.08)
}
...
###fit model
AlpVegRL <- jags(veg.data, veg.inits, 
                c('sd', 'alpha.100', 'gr.ef', 'cb.ef', 'f.ef', 'KK', 'a', 'b'),
                modelfile, n.chains=3, n.iter=20000) #, n.burnin=5000)  



AlpVegRL.upd <- update(AlpVegRL.upd, 
                      params= c('sd', 'alpha', 'gr.ef', 'cb.ef', 'f.ef', 'KK', 'a', 'b'),
                      n.iter=40000, n.thin=15)
print(AlpVegRL)
par(mfrow = c(4, 2))
traceplot(AlpVegRL, mfrow=c(4, 2), varname = c( 'K', 'sd',  'gr.ef', 'cb.ef', 'f.ef', 'a', 'b'), ask=FALSE)
attach.jags(AlpVegRL.upd)
par(mfrow = c(4,2))
plot(density(a[,1]))
plot(density(a[,2]))
plot(density(a[,3]))

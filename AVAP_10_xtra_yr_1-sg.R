...
for (j in 1:Nsites) {
    for (t in yoo1[j] : T) {       
      for (k in 1:Nveg) {
        ...
        mu.K[j,t,k]  <-  KK[j, k] + KT[k] * (t-4) + gr.ef[k] * (1-sg[j, t]) + f.ef[k] * fy.t[j, t] #+ cb.ef[k] * cb.t[j,t] 
...
###fit model
AlpVeg10s <- jags(veg.data, veg.inits,            
                c('KT','sd1','sd2', 'alpha.100', 'gr.ef', 'cb.ef', 'f.ef', 'KK', 'a', 'b'),
                modelfile, n.chains=3, n.iter=10000) 



 AlpVeg10.upd <- update(AlpVeg10s, 
                      params= c('KT','sd1','sd2', 'alpha', 'gr.ef', 'cb.ef', 'f.ef', 'a', 'b', 'mu.K', 'K', 'Er', 'lambda' ),
                      n.iter=10000, n.thin=10)
print(AlpVeg10s)
par(mfrow = c(4, 2))
traceplot(AlpVeg10s, mfrow=c(4, 2), varname = c( 'K', 'sd',  'gr.ef', 'cb.ef', 'f.ef', 'a', 'b'), ask=FALSE)

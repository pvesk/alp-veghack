...
for (k in 1:4) {
  ...
  for (j in 1: Nsites) { 
    for (k in 1: 4) {
      ...
      KK[j,k]  ~ dnorm(mn.K[j,k], sd2^-2)T(0,7.38) #~ dnorm(4,1) #~ dunif(0, 7.08)  #site level departures from predicted K from Precip
}
...
###fit model
AlpVegRLN <- jags(veg.data, veg.inits, 
                c('sd1','sd2', 'sd', 'alpha.100', 'gr.ef', 'cb.ef', 'f.ef', 'KK', 'a', 'b'),
                modelfile, n.chains=3, n.iter=10000) #, n.burnin=5000)  



AlpVegRLN.upd <- update(AlpVegRLN.upd, 
                      params= c('sd1','sd2', 'alpha', 'gr.ef', 'cb.ef', 'f.ef', 'KK', 'a', 'b'),
                      n.iter=50000, n.thin=50)
print(AlpVegRLN)
par(mfrow = c(4, 2))
traceplot(AlpVegRLN, mfrow=c(4, 2), varname = c( 'K', 'sd',  'gr.ef', 'cb.ef', 'f.ef', 'a', 'b'), ask=FALSE)
